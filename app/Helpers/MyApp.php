...
class MyApp
{
  ...
  public static function getConfig($settings, $is_array = false)
  {
    $config = Configuration::where('config_key', $settings)->first();
    if ($is_array && $config) $config->config_value = json_decode($config->config_value, true);
    return $config;
  }

  public static function getCurrency()
  {
    $data = Configuration::where('config_key', 'currency_code')->first();
    return isset($data) ? $data->config_value : "USD";
  }

  public static function getBuseinessName()
  {
    $data = Configuration::where('config_key', 'business_name')->first();
    return isset($data) ? $data->config_value : "Laravel App";
  }

  public static function getBuseinessLogo()
  {
    $data = Configuration::where('config_key', 'fav_icon')->first();
    return isset($data) ? Storage::url($data->config_value) : null;
  }
}
